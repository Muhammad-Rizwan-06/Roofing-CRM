    ...
    useMemo(() => {
      ...
      expenses.forEach((e) => {
        ...
        expenseByCategory[cat] =
          (expenseByCategory[cat] || 0) + Number(e.amount || 0);
      });
      ...
      const agingData = Object.entries(aging).map(([name, value]) => ({
        name,
        value,
      }));

      const expenseCategoryData = Object.entries(expenseByCategory).map(
        ([name, value]) => ({
          name,
          value,
        }),
      ...
      const topProjects = projects
        .map((p) => {
          ...
          const mat = (p.materials || []).reduce(
            (s, m) => s + Number(m.total || 0),
            0,
          );
          const lab = (p.workers || []).reduce(
